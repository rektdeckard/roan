...
public class WorldMap {
    ...
    WorldMap() {
        ...
        scene[5][6][d] = Scene.builder().name(river).disallowed(EnumSet.range(Direction.NE, Direction.SE)).description("To the East, a river runs high and swift from a recent storm. The remains of an abandoned camp at the riverbank have nearly been overwhelmed by the surge waters, but there may be something of use here yet.").detail("A small canvas satchel lies near the waterlogged fire pit. From the weight you can tell it is not empty.").inventory(Weapon.builder().random(WeaponSuffix.DAGGER).description("The dagger is heavier than it appears. Its cold metal stirs beneath the surface, moving like shadows in fog.").build()).build();
        ...
        // THE STEPPE
//            scene[5][7][d] = Scene.builder().name("The Steppe").build();
